...
# ----------------------------
def main():
    ap = argparse.ArgumentParser(description="Baseline vs Lifted: build time, query latencies, timeouts, and per-run sanity checks.")
    ...
    # ---- add this block ----
    ts = time.strftime("%Y%m%d-%H%M%S")  # local time; use UTC with time.gmtime()
    out_path = args.out
    if "{ts}" in out_path:
